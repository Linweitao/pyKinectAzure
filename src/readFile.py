...
if __name__ == "__main__":
    ...
    video_filename = "../data/output0.mkv"
    ...
    while playback.isOpened():
        ...
        if num_bodies > 1:
            continue
        out_file_json = "../pose_result/" + str(index) + ".json"
        ...
        cv2.imwrite("../pose_result/" + str(index) + "_color_image.png", color_image)

        combined_image = color_image
        # Draw the skeletons
        combined_image = body_frame.draw_bodies(combined_image, pykinect.K4A_CALIBRATION_TYPE_COLOR)
        cv2.imwrite("../pose_result/" + str(index) + "_combined_image.png", combined_image)
